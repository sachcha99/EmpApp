...
const EditEmployeeDetails = ({ id, toggle, setToggle }) => {
    ...
    async function getEmployeeDetails() {
        ...
        setPersonalDetails({
            ...
            eid: result.data[0].employeeId,
            nic: result.data[0].nic
        });
    ...
    const handleSubmit = async (event) => {
        ...
        if (data.get('firstName') && data.get('lastName') && data.get('eid') && birthday && data.get('nic') && address && mobile && designation && department) {
            ...
            try {
                const result = await axios.put(`http://localhost:5000/employee/update/${id}`, body)
                setPersonalDetails({
                    firstName: "",
                    lastName: "",
                    eid: "",
                                ...
                                >
                                    ...
                                    <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 3 }}>
                                        <Grid container spacing={2}>
                                            ...
                                            {address && address.map((singleAddress, i) => (
                                            ...
                                            {mobile && mobile.map((singleMobile, i) => (
